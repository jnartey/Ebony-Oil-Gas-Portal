...
if(defined('AWS_STORAGE') && AWS_STORAGE == '1') {
	...
	$filedata = $s3->getObject(AWS_BUCKET, $bucket_path.'voicenote/'.$file);
	$filesize = $filedata->headers['size'];
}else {
	$filepath = dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.'voicenote'.DIRECTORY_SEPARATOR.'uploads'.DIRECTORY_SEPARATOR.($file);
