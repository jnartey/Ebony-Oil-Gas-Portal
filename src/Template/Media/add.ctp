...
<div class="media form large-12 columns">
    ...
    <?= $this->Form->button(__('Upload'), ['class'=>'button', 'id'=>'upload-button']) ?>
    <?= $this->Form->end() ?>
</div>
		...
		$('form').ajaxForm({
			...
			complete: function(xhr) {
				...
				<?php
					if($parent_id){
				?>
					$("#media-container").load("<?php echo $this->Url->build(DS.'media'.DS.'mediaContent'.DS.$parent_id, true); ?>");
				<?php
					}else{
				?>
					$("#media-container").load("<?php echo $this->Url->build(DS.'media'.DS.'mediaContent', true); ?>");
				<?php } ?>
			}
		}); 
