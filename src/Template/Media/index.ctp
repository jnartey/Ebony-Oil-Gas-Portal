					...
					<?php
						if($check_id){
							if($check_parent_media){
								echo '<a href="'.$this->Url->build(DS.'media'.DS.'index'.DS.$parent_media->parent_id, true).'" class="fa fa-level-up"> <span>Move Up</span></a>';
								echo '<a data-fancybox data-type="ajax" data-src="'.$this->Url->build(DS.'media'.DS.'add_folder'.DS.$parent_media->id, true).'" href="javascript:;" class="fa fa-plus"> <span>New Folder</span></a>';
								echo '<a data-fancybox data-type="ajax" href="'.$this->Url->build(DS.'media'.DS.'add'.DS.$parent_media->id, true).'" class="fa fa-upload"> <span>Upload</span></a>';
							}
						}else{
							echo '<a data-fancybox data-type="ajax" data-src="'.$this->Url->build(DS.'media'.DS.'add_folder', true).'" href="javascript:;" class="fa fa-plus"> <span>New Folder</span></a>';
							echo '<a data-fancybox data-type="ajax" href="'.$this->Url->build(DS.'media'.DS.'add', true).'" class="fa fa-upload"> <span>Upload</span></a>';
						}
					?>
			...
			<div id="media-container" class="large-12 columns media-content">
				<nav aria-label="You are here:" role="navigation">
				  <ul class="breadcrumbs">
					  <?php
						  if($crumbs){
							  echo '<li><a href="'.$this->Url->build(DS.'media'.DS.'index', true).'"><span class="fa fa-home"></span></a></li>';
							  foreach($crumbs as $crumb):
								  echo '<li><a href="'.$this->Url->build(DS.'media'.DS.'index'.DS.$crumb->id, true).'">'.$crumb->folder_name.'</a></li>';
							  endforeach;
						  }
					  ?>
				  </ul>
						...
						foreach($media as $data):
							if($data->folder_name){
								...
								if($data->media_access == 1){
									echo '<a href="'.$this->Url->build(DS.'media'.DS.'index'.DS.$data->id, true).'" class="small-6 medium-3 large-2 columns media-folder float-left" title="'.$data->folder_name.'">';
									echo '<div class="icon">';
