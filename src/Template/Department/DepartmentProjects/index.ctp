						...
						<div class="medium-5 columns float-right text-right">
							<?php
								if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
									echo $this->Html->link(__('<span class="fa fa-plus"></span> Create Projects'), ['controller' => 'DepartmentProjects', 'action' => 'add'], ['class'=>'button create-project-btn', 'escape'=>false]);
								}
								echo '&nbsp;';
								echo $this->Html->link(__('<span class="fa fa-tasks"></span> My Tasks'), ['controller' => 'DepartmentTasks', 'action' => 'index'], ['class'=>'button create-project-btn', 'escape'=>false]);
							?>
						</div>
		            </div>
				</div>
	            ...
	            <div class="large-12 columns project-sorting-area">
					 <?php 
					 	foreach ($projects as $project):
							...
							foreach($project->department_project_members as $p_member):
								if($activeUser['id'] == $p_member->user_id){
									$check_p_access = $p_member->user_id;
								}
							endforeach;
							
							if($activeUser['id'] == $check_p_access || $activeUser['id'] == $project->user_id || $department_details->department_role == 2 || $department_details->department_role == 3){
								if($project->status == 1){
									echo '<div class="large-4 columns project incomplete-project"><div class="inner">';
									echo '<a href="'.$this->Url->build('/department/DepartmentProjects/view/'.$project->id, true).'" class="header">';
									...
									if($project->monitor_timeline == 2){
										$task_progress = 0;
										if(!empty($project->department_tasks)){
											foreach($project->department_tasks as $task):
												if($task->status == 3){
									...
									echo '<li>'.$this->cell('Misc::count', ['DepartmentTasks', 'project_id', $project->id, 1])->render('count').' <span>Tasks Left<br>To finish</span></li>';
									echo '<li></li>';
									...
									echo '<li>'.$this->Html->link(__('<span class="fa fa-comments"></span> '.$this->cell('Misc::countComment', ['DepartmentComments', $comment_cat, $project->id])->render('count')), ['controller' => 'DepartmentProjects', 'action' => 'comments', $project->id], ['class'=>'', 'escape'=>false]).'</li>';
									echo '<li>'.$this->Html->link(__('<span class="folders"></span> '.$this->cell('Misc::countFiles', ['DepartmentMedia', null, $project->id, null, $project->department_id])->render('count')), ['controller' => 'DepartmentMedia', 'action' => 'index', $project->department_id], ['class'=>'', 'escape'=>false]).'</li>';
									echo '</ul>';
									echo '</div></div>';
									...
									echo '<div class="large-4 columns project complete-project"><div class="inner">';
									echo '<a href="'.$this->Url->build('/DepartmentProjects/view/'.$project->id, true).'" class="header">';
