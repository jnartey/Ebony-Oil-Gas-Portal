	        ...
	        <section class="large-12 columns event-section">
	            <div class="large-12 columns event-heading">
		            ...
		            <div class="column medium-8 text-right">
						<?php
							echo $this->Html->link(__('Back'), ['controller' => 'Requests', 'action' => 'index'], ['class'=>'button', 'escape'=>false]);
							...
							if($leaveRequest->status != 2 && $leaveRequest->status != 3 && $leaveRequest->status != 4 && $leaveRequest->status != 5){
								if($leaveRequest->user_id == $activeUser['id']){
									echo $this->Html->link(__('Edit'), ['controller' => 'Requests', 'action' => 'edit', $leaveRequest->id], ['class'=>'button']);
									echo $this->Form->postLink(__('Delete'), ['controller' => 'Requests', 'action' => 'delete', $leaveRequest->id], ['confirm' => __('Are you sure you want to delete # {0}?', $leaveRequest->request_type), 'class'=>'button alert']);
								} 
							}
						
							if($activeUser['role_id'] == 1 || $department_details->department_role == 2){
								echo $this->Html->link(__('Recommend'), ['controller' => 'Requests', 'action' => 'recommend', $leaveRequest->id], ['class'=>'button']);
							}
						
							if($department_member->department_role == 3 && $department_member->department->department_type != 3){
							...
							if($leaveRequest->status == 6){
								if($leaveRequest->user_id == $activeUser['id']){
									echo $this->Form->postLink(__('Re-activate'), ['controller' => 'Requests', 'action' => 'set_status', $leaveRequest->id, 1], ['confirm' => __('Are you sure you want to Re-activate {0}?', $leaveRequest->request_type), 'class'=>'button secondary']);
								}
							}elseif($leaveRequest->status == 1){
								if($leaveRequest->user_id == $activeUser['id']){
									echo $this->Form->postLink(__('Cancel'), ['controller' => 'Requests', 'action' => 'set_status', $leaveRequest->id, 6], ['confirm' => __('Are you sure you want to cancel {0}?', $leaveRequest->request_type), 'class'=>'button secondary']);
								}
							}
						?>
					</div>
				...
				<div class="large-12 columns event-content">
					...
					<div class="large-12 columns">
						<div class="large-12 columns portal-sp">
							...
							?>
							<br /><br />
							Leave: <span class="label secondary"><?= $leaveRequest->leave_type ?></span>
							&nbsp;&nbsp;
