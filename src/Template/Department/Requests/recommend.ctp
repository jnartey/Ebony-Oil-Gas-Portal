 ...
 * @link          http://cakephp.org CakePHP(tm) ProjectRequests
 * @since         0.10.0
				...
				<div class="large-12 columns event-content">
					<nav aria-label="You are here:" role="navigation">
					  <ul class="breadcrumbs">
					    ...
					    <li><?= __($leaveRequest->user->name); ?></li>
					  </ul>
					</nav>
					<div class="large-12 columns">
						<div class="large-12 columns portal-sp">
							...
							&nbsp;&nbsp;
							Start date: <span class="label secondary"><?= date('D\, jS F Y', strtotime($leaveRequest->start_date)) ?></span>
							&nbsp;&nbsp;
							End date: <span class="label secondary"><?= date('D\, jS F Y', strtotime($leaveRequest->end_date)) ?></span>
							<br />
							Resumption date: <span class="label secondary"><?= date('D\, jS F Y', strtotime($leaveRequest->resumption_date)) ?></span>
							&nbsp;&nbsp;
							Number of days requested: <span class="label secondary"><?= $leaveRequest->number_of_days_requested ?></span>
							<br /><br />
							<?php
								
								if(isset($relieve->name)){
									echo 'Relieved by: <span class="label secondary">'.$relieve->name.'</span>';
								}
								...
								if($leaveRequest->email){
									echo 'Email: <div class="label secondary">'.$leaveRequest->email.'</div><br /><br />';
								}
								
								if($leaveRequest->address){
									echo 'Address: <div class="callout secondary">'.$leaveRequest->address.'</div>';
								}
							...
							<div class="tasks form large-9 medium-8 columns content float-left">
							    ...
							    <fieldset>
							        <?php
							            ...
							            echo $this->Form->hidden('recommended_by', ['value' => $activeUser['id']]);
										echo $this->Form->hidden('recommended_date', ['value' => date('Y-m-d H:i:s')]);
										echo $this->Form->hidden('status', ['value' => 2]);
										echo '</div>';
							        ?>
							    </fieldset>
								<br />
							    <?= $this->Form->button(__('Recommend'), ['class'=>'button']) ?>
							    <?= $this->Form->end() ?>
							</div>
						</div>
					</div>
