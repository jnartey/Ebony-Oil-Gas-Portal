	        ...
	        <section class="event-section">
	            <div class="event-heading row">
	                ...
	                <div class="column small-9 text-right">
						<?php
							echo $this->Html->link(__('<span class="fa fa-calendar"></span> Event Calendar'), ['controller' => 'DepartmentEvents', 'action' => 'index'], ['class'=>'button', 'escape'=>false]);
							echo $this->Html->link(__('<span class="fa fa-list"></span> All Events'), ['controller' => 'DepartmentEvents', 'action' => 'events'], ['class'=>'button active', 'escape'=>false]);
							if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
								...
								if($activeUser['id'] == $event->user_id){
									echo $this->Html->link(__('<span class="fa fa-pencil"></span> Edit'), ['controller' => 'DepartmentEvents', 'action' => 'edit', $event->id], ['class'=>'button', 'escape'=>false]);
				...
				<div class="large-12 event-content">
					<nav aria-label="You are here:" role="navigation">
					  <ul class="breadcrumbs">
					    <li><?= $this->Html->link(__('Events'), ['controller' => 'DepartmentEvents', 'action' => 'index'], ['escape'=>false]); ?></li>
					    <li><?= $event->name; ?></li>
					  </ul>
					</nav>
					<?php
			        ...
			        <table id="general-table-i" class="display" width="100%" cellpadding="0" cellspacing="0">
						...
						<tbody>
				            ...
				            <tr>
								...
								<td><?php 
									if($data->status == 1){
										echo '<span class="label primary">Attending</span>';
									}else{
										echo '<span class="label primary">Declined</span>';
									}
								?></td>
				                <td class="actions">
				                    ...
				                    <?php 
										if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $event->user_id == $activeUser['id']){
											echo $this->Form->postLink(__('Remove'), ['controller' => 'DepartmentEventsMembers', 'action' => 'delete', $data->id, $event->id], ['confirm' => __('Are you sure you want to remove {0}?', $data->user->name), 'class'=>'button small alert']); 
										}
									?>
				                </td>
				            </tr>
