	        ...
	        <section class="event-section">
				...
				<?php
					foreach ($events as $event):
						echo '<a href="'.$this->Url->build('/department/DepartmentEvents/view/'.$event->id, true).'" class="event-article with-image row">';
						
						$from = Time::parse($event->from_date);
						$to = Time::parse($event->to_date);
						
						foreach($event->department_event_members as $data):
