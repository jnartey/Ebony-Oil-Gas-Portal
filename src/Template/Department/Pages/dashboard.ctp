					...
					<ul>
						<?php
							foreach($projects as $project):
								...
								echo '<li class="large-12 columns"><a class="large-12 columns" href="'.$this->Url->build('/projects/view/'.$project->id, true).'">';
					...
					<ul>
						<?php
							foreach($department_forums as $department_forum):
								...
								$comment_cat = array(4);
								echo '<span class="fa fa-comments-o"> <em>'.$this->cell('Misc::countCommentMisc', ['DepartmentComments', $comment_cat, $department_forum->id, 'forum_id', $department_forum->id, 'department_id', $department_forum->department_id])->render('count').'</em></span>';
								echo '</div></div>';
								echo '</a></li>';
							endforeach;
						?>
