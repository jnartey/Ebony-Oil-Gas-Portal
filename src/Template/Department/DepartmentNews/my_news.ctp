	        ...
	        <section class="event-section">
	                ...
	                <div class="column small-9 text-right">
						<?php
							...
							if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
								echo $this->Html->link(__('<span class="fa fa-list"></span> My News'), ['controller' => 'DepartmentNews', 'action' => 'my_news'], ['class'=>'button active', 'escape'=>false]);
								echo $this->Html->link(__('<span class="fa fa-plus"></span> Add News'), ['controller' => 'DepartmentNews', 'action' => 'add'], ['class'=>'button', 'escape'=>false]);
							}
						?>
					</div>
	            </div>
				<?php
					foreach ($news as $news):
						...
						echo '<a href="'.$this->Url->build('/department/DepartmentNews/view/'.$news->id, true).'" class="event-article with-image row">';
						if($news->image){
