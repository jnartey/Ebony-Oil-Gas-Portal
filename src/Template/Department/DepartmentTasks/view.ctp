	                ...
	                <div class="column small-10 text-right">
						<?php
							...
							if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
								...
								echo $this->Html->link(__('<span class="fa fa-plus"></span> Edit Task'), ['controller' => 'DepartmentTasks', 'action' => 'edit', $task->id], ['class'=>'button', 'escape'=>false]);
							...
							if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
								//echo $this->Html->link(__('Review'), ['controller' => 'DepartmentTasks', 'action' => 'review', $task->id, $activeUser['id']], ['class'=>'button']);
							}
						
							if($task->status != 2 && $task->status != 3){
								echo $this->Form->postLink(__('Done'), ['controller' => 'DepartmentTasks', 'action' => 'set_status', $task->id, $activeUser['id'], 2], ['confirm' => __('Are you sure you want to mark this # {0}?', $task->name.' as done?'), 'class'=>'button alert']);
							}
						?>
					</div>
	            </div>
					...
					<div class="large-12 columns">
						...
						<div class="large-12 columns misc-wrap text-right">
							<?php
								...
								$comment_cat = array(2);
								echo $this->Html->link(__('<span class="fa fa-comments"></span> Comments • '.$this->cell('Misc::countComment', ['DepartmentComments', $comment_cat, $task->id, $project->id])->render('count')), ['controller' => 'DepartmentTasks', 'action' => 'view', $task->id], ['class'=>'', 'escape'=>false]);
							?>
						</div>
						...
						<div id="comments" class="large-12 columns portal-sp comments-box">
							<?php
								$comment_cat = array(2);
								echo '<h6>Comments • '.$this->cell('Misc::countComment', ['DepartmentComments', $comment_cat, $task->id, $project->id])->render('count').'</h6>';
								...
								foreach ($posts as $data):
									...
									if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $activeUser['id'] == $data->user_id){
										...
										echo $this->Html->link(__('Edit'), ['controller' => 'DepartmentTasks', 'action' => 'view', $task->id, $data->id], ['class'=>'', 'escape'=>false]);
										echo ' • ';
										echo $this->Form->postLink(
							                __('Delete'),
