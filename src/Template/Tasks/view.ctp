        ...
        <section class="large-12 columns event-section">
            <div class="large-12 columns event-heading">
                ...
                <div class="column small-10 text-right">
					<?php
						echo $this->Html->link(__('Back'), ['controller' => 'projects', 'action' => 'view', $project->id], ['class'=>'button', 'escape'=>false]);
						if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
							...
							echo $this->Html->link(__('<span class="fa fa-plus"></span> Create Task'), ['controller' => 'tasks', 'action' => 'add', $project->id], ['class'=>'button', 'escape'=>false]);
							echo $this->Html->link(__('<span class="fa fa-plus"></span> Edit Task'), ['controller' => 'tasks', 'action' => 'edit', $project->id], ['class'=>'button', 'escape'=>false]);
						...
						if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
							//echo $this->Html->link(__('Review'), ['controller' => 'tasks', 'action' => 'review', $task->id, $activeUser['id']], ['class'=>'button']);
						}
						
						if($task->status != 2 && $task->status != 3){
							echo $this->Form->postLink(__('Done'), ['controller' => 'tasks', 'action' => 'set_status', $task->id, $activeUser['id'], 2], ['confirm' => __('Are you sure you want to mark this # {0}?', $task->name.' as done?'), 'class'=>'button alert']);
						}
					?>
				</div>
            </div>
			<div class="large-12 columns event-content">
				<nav aria-label="You are here:" role="navigation">
				  <ul class="breadcrumbs">
				    ...
				    <li><?= $this->Html->link(__($project->name), ['controller' => 'projects', 'action' => 'view', $project->id], ['escape'=>false]); ?></li>
					<li><?= __('Task'); ?></li>
				  </ul>
				</nav>
				<div class="large-12 columns">
					...
					<div class="large-12 columns misc-wrap text-right">
						<?php
							...
							echo $this->Html->link(__('<span class="fa fa-comments"></span> Comments • '.$this->cell('Misc::count', ['Comments', 'comment_src', 2])->render('count')), ['controller' => 'tasks', 'action' => 'view', $task->id, '#'=>'comments'], ['class'=>'', 'escape'=>false]);
						?>
					</div>
					...
					<?php }else{
							...
							echo $this->Html->link(__('Cancel'), ['controller' => 'tasks', 'action' => 'view', $task->id], ['class'=>'button', 'escape'=>false]);
							echo $this->Form->end();
						} 
					?>
					</div>
					<div id="comments" class="large-12 columns portal-sp comments-box">
						<h6>Comments • <?= $count = $this->cell('Misc::count', ['Comments', 'comment_src', 2])->render('count'); ?></h6>
						<?php
							foreach ($posts as $data):
								...
								if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $activeUser['id'] == $data->user_id){
									...
									echo $this->Html->link(__('Edit'), ['controller' => 'tasks', 'action' => 'view', $task->id, $data->id], ['class'=>'', 'escape'=>false]);
									echo ' • ';
									echo $this->Form->postLink(
						                __('Delete'),
