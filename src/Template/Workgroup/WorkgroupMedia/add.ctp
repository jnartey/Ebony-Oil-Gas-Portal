		...
		$('form').ajaxForm({
			...
			complete: function(xhr) {
				...
				<?php
					if($parent_id){
				?>
					$("#media-container").load("<?php echo $this->Url->build(DS.'workgroup'.DS.'workgroup-media'.DS.'mediaContent'.DS.$parent_id, true); ?>");
				<?php
					}else{
				?>
					$("#media-container").load("<?php echo $this->Url->build(DS.'workgroup'.DS.'workgroup-media'.DS.'mediaContent', true); ?>");
				<?php } ?>
			}
		}); 
