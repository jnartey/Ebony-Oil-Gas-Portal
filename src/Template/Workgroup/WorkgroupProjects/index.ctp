						...
						<div class="medium-5 columns float-right text-right">
							<?php
								if($activeUser['role_id'] == 1 || $workgroup_details->user_id == $activeUser['id']){
									echo $this->Html->link(__('<span class="fa fa-plus"></span> Create Projects'), ['controller' => 'WorkgroupProjects', 'action' => 'add'], ['class'=>'button create-project-btn', 'escape'=>false]);
								}
								echo '&nbsp;';
								echo $this->Html->link(__('<span class="fa fa-plus"></span> My Tasks'), ['controller' => 'WorkgroupTasks', 'action' => 'index'], ['class'=>'button create-project-btn', 'escape'=>false]);
							?>
						</div>
		            </div>
				</div>
					 ...
					 <?php 
					 	foreach ($projects as $project):
							...
							if($project->status == 1){
								...
								echo '<a href="'.$this->Url->build('/workgroup/WorkgroupProjects/view/'.$project->id, true).'" class="header">';
								...
								if($project->monitor_timeline == 2){
									$task_progress = 0;
									if(!empty($project->WorkgroupTasks)){
										foreach($project->WorkgroupTasks as $task):
											if($task->status == 3){
								...
								echo '<li>'.$this->cell('Misc::count', ['WorkgroupTasks', 'project_id', $project->id, 0])->render('count').' <span>Tasks Left<br>To finish</span></li>';
								echo '<li></li>';
								...
								echo '<li>'.$this->Html->link(__('<span class="fa fa-comments"></span> '.$this->cell('Misc::countComment', ['WorkgroupComments', $comment_cat, $project->id])->render('count')), ['controller' => 'WorkgroupProjects', 'action' => 'comments', $project->id], ['class'=>'', 'escape'=>false]).'</li>';
								echo '<li>'.$this->Html->link(__('<span class="folders"></span> '.$this->cell('Misc::countFiles', ['WorkgroupMedia', null, $project->id, null, null, $project->workgroup_id])->render('count')), ['controller' => 'WorkgroupMedia', 'action' => 'index', $project->id], ['class'=>'', 'escape'=>false]).'</li>';
								echo '</ul>';
								echo '</div></div>';
								...
								echo '<div class="large-4 columns project complete-project"><div class="inner">';
								echo '<a href="'.$this->Url->build('/WorkgroupProjects/view/'.$project->id, true).'" class="header">';
