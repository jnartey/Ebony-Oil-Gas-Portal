	        ...
	        <section class="large-12 columns event-section">
	            <div class="large-12 columns event-heading">
		            ...
		            <div class="column medium-8 text-right">
						<?php
							echo $this->Html->link(__('Back'), ['controller' => 'WorkgroupWiki', 'action' => 'index'], ['class'=>'button', 'escape'=>false]);
							if($activeUser['role_id'] == 1 || $workgroup_details->user_id == $activeUser['id']){
								echo $this->Html->link(__('Add Wiki'), ['controller' => 'WorkgroupWiki', 'action' => 'add'], ['class'=>'button', 'escape'=>false]);
								echo $this->Html->link(__('Edit'), ['controller' => 'WorkgroupWiki', 'action' => 'edit', $wiki->id], ['class'=>'button']);
								echo $this->Form->postLink(__('Delete'), ['controller' => 'WorkgroupWiki', 'action' => 'delete', $wiki->id], ['confirm' => __('Are you sure you want to delete # {0}?', $wiki->title), 'class'=>'button alert']);
							}
						?>
					</div>
	            </div>
						...
						<div class="large-12 columns portal-sp">
							<div class="large-12 columns">
								<?php
									...
									echo '<a data-fancybox data-type="ajax" href="'.$this->Url->build(DS.'workgroup'.DS.'workgroup-wiki'.DS.'upload'.DS.$wiki->id, true).'" class="button"> <span class="fa fa-upload"></span> Upload file</a>';
								?>
							</div>
