						...
						<div class="large-12 columns portal-sp space-button">
					        <table id="general-table" class="display" width="100%" cellpadding="0" cellspacing="0">
								...
								<tbody>
						            ...
						            <tr>
						                <td><?= h($i) ?></td>
						                <td><?= $this->Html->link($data->title, ['controller' => 'WorkgroupWiki', 'action' => 'view', $data->id]); ?></td>
										<td><?= $data->created; ?></td>
						                <td class="actions">
						                    <?php 
											echo $this->Html->link(__('View'), ['controller' => 'WorkgroupWiki', 'action' => 'view', $data->id], ['class'=>'button small']);
										
											if($activeUser['role_id'] == 1 || $workgroup_details->user_id == $activeUser['id']){
													if($data->user_id == $activeUser['id']){
														echo $this->Html->link(__('Edit'), ['controller' => 'WorkgroupWiki', 'action' => 'edit', $data->id], ['class'=>'button small']);
														echo $this->Form->postLink(__('Delete'), ['controller' => 'WorkgroupWiki', 'action' => 'delete', $data->id], ['confirm' => __('Are you sure you want to delete # {0}?', $data->title), 'class'=>'button small alert']);
													} 
												}
											?>
						                </td>
