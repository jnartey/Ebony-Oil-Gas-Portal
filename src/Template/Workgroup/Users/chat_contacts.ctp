			...
			if(!empty($user->users_log)){
				foreach($user->users_log as $data):
					if($user->modified == $data->created){
						...
						echo '<li>';
						echo '<a href="javascript:void(0)" onclick="javascript:chatWith(\''.$user->username.'\')">';
						if($data->status == 2){
							if($user->photo){
								echo '<span class="avatar" style="background-image: url('.$this->Url->build(DS.$user->photo_dir.DS.'small-'.$user->photo, true).')">';
								echo '<span class="is-online"></span></span>';
							}else{
								echo '<span class="avatar" style="background-image: url('.$this->Url->build('/img/dummy.png', true).')">';
								echo '<span class="is-online"></span></span>';
							}
							echo '<div><h6>'.$user->name.'</h6><time>'.$logout_time->timeAgoInWords().'</time></div>';
							...
							echo '<div><h6>'.$user->name.'</h6><time>'.$login_time->timeAgoInWords().'</time></div>';
						}else{
							if($user->photo){
								echo '<span class="avatar" style="background-image: url('.$this->Url->build(DS.$user->photo_dir.DS.'small-'.$user->photo, true).')">';
								echo '<span class="is-online offline"></span></span>';
