	        ...
	        <section class="large-12 columns event-section">
	            <div class="large-12 columns event-heading">
	                ...
	                <div class="column small-10 text-right">
						<?php
							echo $this->Html->link(__('Back'), ['controller' => 'WorkgroupForums', 'action' => 'index'], ['class'=>'button']);
							if($activeUser['role_id'] == 1 || $workgroup_details->workgroup_role == 2 || $workgroup_details->workgroup_role == 3 || $workgroupsForum->user_id == $activeUser['id']){
								echo $this->Html->link(__('Edit'), ['controller' => 'WorkgroupForums', 'action' => 'edit', $workgroupsForum->id], ['class'=>'button']);
								echo $this->Form->postLink(__('Delete'), ['controller' => 'WorkgroupForums', 'action' => 'delete', $workgroupsForum->id], ['confirm' => __('Are you sure you want to delete # {0}?', $workgroupsForum->title), 'class'=>'button alert']);
							}
						?>
					</div>
	            </div>
				<div class="large-12 columns event-content">
					<nav aria-label="You are here:" role="navigation">
					  <ul class="breadcrumbs">
					    <li><?= $this->Html->link(__('Forum'), ['controller' => 'WorkgroupForums', 'action' => 'index'], ['escape'=>false]); ?></li>
					    <li><?= __($workgroupsForum->title); ?></li>
					  </ul>
					</nav>
					<div class="large-12 columns">
						<span class="label secondary"><strong>Created by:</strong> <?= $workgroupsForum->user->name ?></span><br /><br />
					   	<?= $workgroupsForum->description ?>
						<div id="comments" class="large-12 columns portal-sp comments-box">
					        ...
					        <?php
								...
								echo $this->Form->hidden('source_id', ['value' => $workgroupsForum->id]);
					            echo $this->Form->hidden('forum_id', ['value' => $workgroupsForum->id]);
							...
							<?php }else{
									...
									echo '&nbsp;';
									echo $this->Html->link(__('Cancel'), ['controller' => 'tasks', 'action' => 'view', $workgroupsForum->id], ['class'=>'button', 'escape'=>false]);
									echo $this->Form->end();
								} 
								
								$comment_cat = array(3);
								echo '<h6>Comments • '.$this->cell('Misc::countCommentMisc', ['WorkgroupComments', $comment_cat, $data->id, 'forum_id', $workgroupsForum->id, 'workgroup_id', $workgroupsForum->workgroup_id])->render('count').'</h6>';
								...
								foreach ($posts as $data):
									...
									if($activeUser['role_id'] == 1 || $workgroup_details->workgroup_role == 2 || $workgroup_details->workgroup_role == 3 || $activeUser['id'] == $data->user_id){
										...
										echo $this->Html->link(__('Edit'), ['controller' => 'WorkgroupsForums', 'action' => 'view', $workgroupsForum->id, $data->id], ['class'=>'', 'escape'=>false]);
										echo ' • ';
