	                ...
	                <div class="column small-9 text-right">
						<?php
							...
							if($activeUser['role_id'] == 1 || $workgroup_details->user_id == $activeUser['id']){
								echo $this->Html->link(__('<span class="fa fa-list"></span> My Events'), ['controller' => 'WorkgroupEvents', 'action' => 'my_events'], ['class'=>'button active', 'escape'=>false]);
				...
				<?php
					if($events){
						foreach ($events as $event):
							...
							echo '<a href="'.$this->Url->build('/workgroup/WorkgroupEvents/view/'.$event->id, true).'" class="event-article with-image row">';
							if($event->image){
