	        ...
	        <section class="event-section">
	                ...
	                <div class="column small-9 text-right">
						<?php
							...
							if($activeUser['role_id'] == 1 || $workgroup_details->user_id == $activeUser['id']){
								echo $this->Html->link(__('<span class="fa fa-list"></span> My News'), ['controller' => 'WorkgroupNews', 'action' => 'my_news'], ['class'=>'button active', 'escape'=>false]);
				...
				<?php
					foreach ($news as $news):
						...
						echo '<a href="'.$this->Url->build('/workgroup/WorkgroupNews/view/'.$news->id, true).'" class="event-article with-image row">';
						if($news->image){
