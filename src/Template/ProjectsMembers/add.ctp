                ...
                <div class="column small-9 text-right">
					<?php
						if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
							echo $this->Html->link(__('<span class="fa fa-list"></span> Back'), ['controller' => 'projects', 'action' => 'view', $project->id], ['class'=>'button', 'escape'=>false]);
							echo $this->Html->link(__('<span class="fa fa-edit"></span> Edit Project'), ['controller' => 'projects', 'action' => 'edit', $project->id], ['class'=>'button', 'escape'=>false]);
						}
						echo $this->Html->link(__('<span class="fa fa-plus"></span> Tasks'), ['controller' => 'tasks', 'action' => 'view', $activeUser['id']], ['class'=>'button', 'escape'=>false]);
					?>
				</div>
            </div>
				...
				<div class="large-12 columns">
					...
					<div class="large-12 columns portal-sp">
						<div class="tasks form large-12 medium-10 columns content">
						    <?= $this->Form->create($projectsMember) ?>
						    <fieldset>
						        <h4><?= __('Add Project Staff') ?></h4>
						        <?php
