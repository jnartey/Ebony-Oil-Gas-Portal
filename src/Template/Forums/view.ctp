        ...
        <section class="large-12 columns event-section">
            <div class="large-12 columns event-heading">
                ...
                <div class="column small-10 text-right">
					<?php
						echo $this->Html->link(__('Back'), ['controller' => 'Forums', 'action' => 'index'], ['class'=>'button']);
						if($activeUser['id'] == $departmentsForum->user_id){
							echo $this->Html->link(__('Edit'), ['controller' => 'Forums', 'action' => 'edit', $departmentsForum->id], ['class'=>'button']);
							echo $this->Form->postLink(__('Delete'), ['controller' => 'Forums', 'action' => 'delete', $departmentsForum->id], ['confirm' => __('Are you sure you want to delete # {0}?', $departmentsForum->title), 'class'=>'button alert']);
						}
					?>
				</div>
            </div>
			<div class="large-12 columns event-content">
				<nav aria-label="You are here:" role="navigation">
				  <ul class="breadcrumbs">
				    <li><?= $this->Html->link(__('Forum'), ['controller' => 'Forums', 'action' => 'index'], ['escape'=>false]); ?></li>
				...
				<div class="large-12 columns">
					...
					<div id="comments" class="large-12 columns portal-sp comments-box">
						...
						?>
						<h6>Comments • <?= $count = $this->cell('Misc::count', ['Comments', 'comment_src', 4])->render('count'); ?></h6>
						<?php
							foreach ($posts as $data):
								...
								if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $activeUser['id'] == $data->user_id){
									...
									echo $this->Html->link(__('Edit'), ['controller' => 'Forums', 'action' => 'view', $departmentsForum->id, $data->id], ['class'=>'', 'escape'=>false]);
									echo ' • ';
