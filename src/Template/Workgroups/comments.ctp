        ...
        <section class="large-12 columns event-section">
                ...
                <div class="column small-10 text-right">
					<?php
						if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $workgroup->user_id == $activeUser['id']){
							echo $this->Html->link(__('Back'), ['controller' => 'workgroups', 'action' => 'view', $workgroup->id], ['class'=>'button', 'escape'=>false]);
			...
			<div class="large-12 columns event-content">
				...
				<div class="large-12 columns">
			        ...
			        <?php
			            echo $this->Form->hidden('comment_src', ['value' => 3]);
						echo $this->Form->hidden('source_id', ['value' => $workgroup->id]);
					...
					<?php }else{
							...
							echo '&nbsp;';
							echo $this->Html->link(__('Cancel'), ['controller' => 'workgroups', 'action' => 'comments', $workgroup->id], ['class'=>'button', 'escape'=>false]);
							echo $this->Form->end();
						} 
					?>
					<div id="comments" class="large-12 columns portal-sp comments-box">
						<h6>Comments • <?= $count = $this->cell('Misc::count', ['Comments', 'comment_src', 3])->render('count'); ?></h6>
						<?php
							foreach ($posts as $data):
								...
								if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3 || $activeUser['id'] == $data->user_id){
									...
									echo $this->Html->link(__('Edit'), ['controller' => 'workgroups', 'action' => 'comments', $workgroup->id, $data->id], ['class'=>'', 'escape'=>false]);
									echo ' • ';
									echo $this->Form->postLink(
						                __('Delete'),
