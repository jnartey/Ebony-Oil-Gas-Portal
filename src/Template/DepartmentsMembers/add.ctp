    ...
    <div class="column small-12 large-9">
        <section class="large-12 columns event-section">
            <div class="large-12 columns event-heading">
                <h1 class="column small-3">Department</h1>
                <div class="column small-9 text-right">
					<?php
						if($activeUser['role_id'] == 1 || $department_details->department_role == 2 || $department_details->department_role == 3){
							echo $this->Html->link(__('<span class="fa fa-list"></span> Back'), ['controller' => 'departments', 'action' => 'index'], ['class'=>'button', 'escape'=>false]);
							//echo $this->Html->link(__('<span class="fa fa-edit"></span> Edit Project'), ['controller' => 'departments', 'action' => 'edit', $department->id], ['class'=>'button', 'escape'=>false]);
						}
					?>
				</div>
            </div>
			<div class="large-12 columns event-content">
				<nav aria-label="You are here:" role="navigation">
				  <ul class="breadcrumbs">
				    <li><?= $this->Html->link(__('Departments'), ['controller' => 'departments', 'action' => 'index'], ['escape'=>false]); ?></li>
				    <li><?= __($department->name); ?></li>
				  </ul>
				</nav>
						...
						<div class="tasks form large-12 medium-10 columns content">
						    ...
						    <fieldset>
						        ...
						        <?php
									...
									echo $this->Form->hidden('department_id', ['value' => $department->id]);
						        ?>
						    </fieldset>
						    <?= $this->Form->button(__('Add Staff'), ['class'=>'button']) ?>
						    <?= $this->Form->end() ?>
						</div>
					</div>
				</div>
